...
2. Attempt scoped synchronization before search: `<cli> sync --project "..." --time current --semantic auto --json`. If sync is skipped or busy, proceed directly to search; search uses read-only overlay for unindexed files.
3. Run `<cli> search --query "..." --project "..." --time current --json`.
4. If the result says `ambiguous` or `unknown`, stop and ask the user which project they mean. Do not search several similar projects together.
5. Use only returned evidence whose relative path and line range can be reopened. Treat similarity as relevance, never as factual confidence.
6. For prices, commitments, status, account, permission, publication, deletion, or other high-impact facts, require a current authoritative source. If it is missing, answer that evidence is insufficient.
7. If current sources conflict, show both with their citations and ask for a decision. Do not silently choose one.
...
6. When replacing a fact, mark the older record as superseded. Keep history; do not delete it to make the new answer appear certain.
...
Prefer the JSON CLI for one-shot work. Use the local stdio MCP server for another Agent runtime. The MCP write surface exposes candidate creation and bounded synchronization (`second_brain_sync`); confirmation and activation remain deliberate CLI/user-governed steps.
...
- Do not start background indexing daemons or background file watchers.
- Do not modify the vault during search, sync, or health checks.
